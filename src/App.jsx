    ...
    <div className={`min-h-screen font-sans selection:bg-orange-accent selection:text-white pb-20 transition-colors duration-300 ${
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-6 sm:gap-8">
            ...
            <nav className="hidden md:flex items-center gap-6 text-xs sm:text-sm font-medium text-muted-text">
              ...
              <a href="#schedule" className="hover:text-charcoal-text transition-colors">Leaderboard</a>
              <a href="#community" className="hover:text-charcoal-text transition-colors">Community</a>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden xs:flex items-center gap-1 rounded-full bg-charcoal text-cream px-2.5 py-1 text-xs font-bold">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-12 pt-6 sm:pt-10 lg:pt-14 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {/* Card 1: Custom Player Settings */}
          <div
            onClick={() => setIsPlayerModalOpen(true)}
            className="rounded-[24px] border border-border-cream bg-surface-white p-6 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-card group"
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-bold text-orange-accent">{playerSpeed}</span>
            </div>
            <h3 className="font-serif text-lg font-medium text-charcoal-text group-hover:text-orange-accent transition-colors">
              Custom Video Player
            </h3>
            <p className="mt-1 text-xs text-muted-text leading-relaxed">
              Adjust playback speeds ({playerSpeed}) and toggle study light/dark themes.
            </p>
          </div>

          {/* Card 2: Daily Streak */}
          <div className="rounded-[24px] bg-charcoal p-6 text-cream flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-orange-accent uppercase tracking-wider">Active Streak</span>
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-orange-accent text-white font-bold text-xs">
                {streakDays}d
              </span>
            </div>
            <div className="mt-4">
              <h3 className="font-serif text-lg font-medium text-white">Daily Learning Alerts</h3>
              <p className="mt-1 text-xs text-cream/70 leading-relaxed">
                Receive friendly practice check reminders to maintain your momentum.
              </p>
            </div>
          </div>

          {/* Card 3: Multi-device sync */}
          <div className="rounded-[24px] border border-border-cream bg-surface-beige p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-white text-xl border border-border-cream mb-4">
              📱
            </div>
            <h3 className="font-serif text-lg font-medium text-charcoal-text">Learn Anywhere</h3>
            <p className="mt-1 text-xs text-muted-text leading-relaxed">
              Your XP, daily streak, and progress automatically sync across desktop, tablet, and mobile.
            </p>
          </div>
        </div>
